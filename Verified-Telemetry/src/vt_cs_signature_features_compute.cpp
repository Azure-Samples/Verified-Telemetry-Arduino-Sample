...
//     Serial.printf("less peaks - NOT TAKEN\n");
//     return 0;
// }
if(peaks<2){
    #if VT_LOG_LEVEL > 3
...
{
    static VT_FLOAT acr_signal[VT_CS_SAMPLE_LENGTH] = {0};
    memset(acr_signal,0,sizeof(VT_FLOAT)*VT_CS_SAMPLE_LENGTH);
    //    VT_INT decimal;
    //VT_FLOAT frac_float;
    //VT_INT frac;
...
autocorrelation(raw_signature, acr_signal);
    ...
    static COMPLEX signal[128];
    memset(signal,0,sizeof(COMPLEX)*128);

    static SPECTOGRAM spectogram_calib_fetch[64];
    memset(spectogram_calib_fetch,0,sizeof(SPECTOGRAM)*64);
    ...
    VT_UINT mode)
{
	...
	int* clustercounts=(int *)calloc(config.k,sizeof(int));
	float* dominant=(float *)calloc(config.num_objs,sizeof(float));
...
Serial.printf("\n");
		...
		for (i = 0; i < config.num_objs-1; i++)
	{
		if (config.objs[i])
            ...
            #if VT_LOG_LEVEL > 3
			Serial.printf("%d,",config.clusters[i]);
            #endif
		//else
			//Serial.printf("NN [%d]\n", config.clusters[i]);
	}
    ...
    VT_FLOAT* current_average)
{
    ...
    binary_state_current_compute(
            raw_signature, category_array,raw_signature_length, &curr_draw_active, &curr_draw_standby, &datapoints_active, &datapoints_standby,1);
...
*relative_current_draw = curr_draw_active - curr_draw_standby;
    ...
    if (period_calculate(raw_signature_copy, betasigfreqs) == VT_SUCCESS)
    {
        ...
        frac       = fabsf(frac_float) * 10000;
        VTLogDebug("Repeating Signature Relative Curr current average = %d.%04d\r\n", decimal, frac);
#endif /* VT_LOG_LEVEL > 2 */
...
VTLogDebugNoTag("\n");
    ...
    binary_state_current_compute(
        raw_signature, category_array,raw_signature_length, &curr_draw_active, &curr_draw_standby, &datapoints_active, &datapoints_standby,0);
