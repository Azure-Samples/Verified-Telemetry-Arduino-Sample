...
{   //Serial.printf("cs_downsample \n");
    ...
    //     Serial.printf("%d.%04d, ", decimal, frac);
        ...
        raw_signature_buffer->num_datapoints++;
        //Serial.printf("num_datapoints - %d ",raw_signature_buffer->num_datapoints);
        if (raw_signature_buffer->num_datapoints == raw_signature_buffer->sample_length)
        {   //HEREprintf("full \n");
...
static VT_VOID cs_downsample_non_repeating_raw_signature_buffer()
{
    ...
    */
    ...
    //Serial.printf("out of downsample\r");
    ...
    //Serial.printf("sampling_frequency_temp before %d \n",sampling_frequency_temp);

    cs_object_reference->raw_signatures_reader->non_repeating_raw_signature.num_datapoints = samples_stored_after_modification;

    //Serial.printf("non rep num datapoints : %d \n",cs_object_reference->raw_signatures_reader->non_repeating_raw_signature.num_datapoints);

    if (cs_object_reference->raw_signatures_reader->non_repeating_raw_signature.num_datapoints==VT_CS_SAMPLE_LENGTH)
    {   
        cs_object_reference->raw_signatures_reader->non_repeating_raw_signature_stop_collection = true;
    }

    //Serial.printf("samples_stored_after_modification %d \n",samples_stored_after_modification);
    /*
    cs_object_reference->raw_signatures_reader->non_repeating_raw_signature.sampling_frequency =
        cs_object_reference->raw_signatures_reader->adc_read_sampling_frequency / downsample_factor;
    //Serial.printf("adc_read_sampling_frequency %d \n",(int)cs_object_reference->raw_signatures_reader->adc_read_sampling_frequency);
    */
...
{   //Serial.printf("full callback\n");
    // VTLogDebug("cs_raw_signature_read_full_complete_callback() \r\n");
    //Serial.println("FCC");

    //Serial.printf(" %d, \n",cs_object_reference->raw_signatures_reader->non_repeating_raw_signature.num_datapoints);
    //Serial.printf(" %d, ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[0].num_datapoints);
    //Serial.printf(" %d, ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[1].num_datapoints);
    //Serial.printf(" %d, ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[2].num_datapoints);
    //Serial.printf(" %d, ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[3].num_datapoints);
    //Serial.printf(" %d, \n",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[4].num_datapoints);
    ...
    //Serial.printf(" buffer read call again \n");
    ...
    if (cs_object_reference->raw_signatures_reader->repeating_raw_signature_buffers_filled)
    {   
        //Serial.printf(" repeating_raw_signature_buffers_filled \n");
        return stop_read;
    }
    ...
    //Serial.printf("non rep num datapoints : %d \n",cs_object_reference->raw_signatures_reader->non_repeating_raw_signature.num_datapoints);
    //Serial.printf("index 0 : %d ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[0].num_datapoints);
    //Serial.printf("index 1 : %d ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[1].num_datapoints);
    //Serial.printf("index 2 : %d ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[2].num_datapoints);
    //Serial.printf("index 3 : %d ",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[3].num_datapoints);
    //Serial.printf("index 4 : %d \n",cs_object_reference->raw_signatures_reader->repeating_raw_signatures[4].num_datapoints);


        cs_object_reference->raw_signatures_reader->repeating_raw_signature_ongoing_collection = false;
        cs_object_reference->raw_signatures_reader->repeating_raw_signature_buffers_filled     = true;
        stop_read=true;
        //Serial.printf(" buffer read 2 \n");
        return stop_read;
    }
/*
    if (mode==1){
...
*/
    //Serial.printf(" buffer read finished \n");

    /* Transfer data from adc buffer to non repeating signature buffer */
    // VTLogDebug("cs_adc_buffer_to_non_repeating_raw_signature_buffer() \r\n");
    //Serial.printf(" buffer read 3 \n");
    return stop_read;
}
    ...
    VT_CURRENTSENSE_OBJECT* cs_object, VT_FLOAT desired_sampling_frequency, VT_FLOAT* closest_floor_sampling_frequency)
{
    ...
    {   
        #if VT_LOG_LEVEL > 3
        Serial.printf("error in closest floor");
        #endif
