    ...
    VT_FLOAT* sec_sig_freq)
{
    ...
    if (signature_iter >= cs_object->fingerprintdb.template_struct.repeating_signatures.num_signatures)
    {
        VTLogDebug("Next repeating signature cannot be fetched from DB\r\n");
        return VT_ERROR;
    }

    *sampling_frequency    = cs_object->fingerprintdb.template_struct.repeating_signatures.signatures[signature_iter].sampling_freq;
    *signature_frequency   = cs_object->fingerprintdb.template_struct.repeating_signatures.signatures[signature_iter].signature_freq;
    *sec_sig_freq   = cs_object->fingerprintdb.template_struct.repeating_signatures.signatures[signature_iter].sec_signature_freq;
    *duty_cycle            = cs_object->fingerprintdb.template_struct.repeating_signatures.signatures[signature_iter].duty_cycle;
    *relative_current_draw = cs_object->fingerprintdb.template_struct.repeating_signatures.signatures[signature_iter].relative_curr_draw;
    return VT_SUCCESS;
}
...
{
    if (cs_object->fingerprintdb.template_struct.repeating_signatures.offset_current == VT_DATA_NOT_AVAILABLE)
    {
        VTLogDebug("Offset Current of repeating signatures unavailable in DB\r\n");
        return VT_ERROR;
    }
    *lowest_sample_freq = cs_object->fingerprintdb.template_struct.repeating_signatures.lowest_sample_freq;
    *offset_current     = cs_object->fingerprintdb.template_struct.repeating_signatures.offset_current;
    return VT_SUCCESS;
}
    ...
    VT_CURRENTSENSE_OBJECT* cs_object, VT_FLOAT* avg_curr_on, VT_FLOAT* avg_curr_off)
{
    ...
    if (cs_object->fingerprintdb.template_struct.non_repeating_signature.avg_curr_on == VT_DATA_NOT_AVAILABLE ||
    ...
    *avg_curr_on  = cs_object->fingerprintdb.template_struct.non_repeating_signature.avg_curr_on;
    *avg_curr_off = cs_object->fingerprintdb.template_struct.non_repeating_signature.avg_curr_off;
    ...
    VT_UINT* num_repeating_signature_sampling_frequencies)
{
    ...
    *num_repeating_signature_sampling_frequencies = 0;
    ...
    VT_UINT iter=0;
    for (iter = 0; iter < cs_object->fingerprintdb.template_struct.repeating_signatures.num_signatures; iter++)
    {
    ...
    frac_float = cs_object->fingerprintdb.template_struct.repeating_signatures.signatures[iter].sampling_freq - (VT_FLOAT)decimal;
    frac       = fabsf(frac_float) * 10000;
    VTLogDebug("freq taken = %d.%04d\r\n", decimal, frac);
#endif /* VT_LOG_LEVEL > 2 */

        repeating_signature_sampling_freqeuencies[iter] =
            cs_object->fingerprintdb.template_struct.repeating_signatures.signatures[iter].sampling_freq;
        *num_repeating_signature_sampling_frequencies = *num_repeating_signature_sampling_frequencies + 1;
    }
    repeating_signature_sampling_freqeuencies[iter] = cs_object->fingerprintdb.template_struct.repeating_signatures.lowest_sample_freq;
    *num_repeating_signature_sampling_frequencies = *num_repeating_signature_sampling_frequencies + 1;
            #if VT_LOG_LEVEL > 3
