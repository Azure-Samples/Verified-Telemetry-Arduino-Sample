...
VT_FLOAT fc_calculate_falltime_deviation(VT_ULONG falltime_under_test, VT_ULONG falltime_saved)
{
    VT_FLOAT falltime_deviation =
        (fabsf((VT_FLOAT)falltime_under_test - (VT_FLOAT)falltime_saved) / (VT_FLOAT)falltime_saved) * 100.0f;
...
{
    VT_FLOAT pearson_coeff_deviation = (fabsf(pearson_coeff_under_test - pearson_coeff_saved) / pearson_coeff_saved) * 100.0f;
