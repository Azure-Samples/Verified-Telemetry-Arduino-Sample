...
typedef struct VT_CURRENTSENSE_SIGNATURES_DEBUG_STRUCT {

    VT_FLOAT non_rep_current_drift;
    VT_BOOL non_rep_drift_valid;

    VT_FLOAT offset_current_drift;
    VT_BOOL offset_current_drift_valid;

    VT_FLOAT rep_sampling_freq[VT_CS_MAX_SIGNATURES];

    VT_FLOAT rep_feature_vector_drift[VT_CS_MAX_SIGNATURES];
    VT_BOOL rep_feature_vector_drift_valid[VT_CS_MAX_SIGNATURES];

    VT_FLOAT rep_relative_current_drift[VT_CS_MAX_SIGNATURES];
    VT_BOOL rep_relative_current_drift_valid[VT_CS_MAX_SIGNATURES];

}VT_CURRENTSENSE_SIGNATURES_DEBUG;

typedef struct VT_CURRENTSENSE_DATABASE_STRUCT
{
    VT_UINT template_type;
    VT_CURRENTSENSE_SIGNATURES_TEMPLATE template_struct;
    VT_CURRENTSENSE_SIGNATURES_DEBUG debug_template;
...
// Fetch Database and template_struct confidence
