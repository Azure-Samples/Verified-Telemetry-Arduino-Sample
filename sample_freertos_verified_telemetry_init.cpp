...
int vt_analog_sensor_signature_init(FreeRTOS_VERIFIED_TELEMETRY_DB* verified_telemetry_DB,
                                            ...
                                            VT_SENSOR_HANDLE* sensor_handle)
{
    ...
    if ((status = FreeRTOS_vt_signature_init(verified_telemetry_DB,
             ...
             sensor_handle)))
    {   
        Serial.printf("Failed to initialize VT for soilMoistureExternal1 telemetry: error code = 0x%08x\r\n", status);
        return VT_ERROR;
    }
...
int vt_digital_sensor_signature_init(FreeRTOS_VERIFIED_TELEMETRY_DB* verified_telemetry_DB,
                                            FreeRTOS_VT_OBJECT* handle,
                                            UCHAR* component_name_ptr,
                                            bool telemetry_status_auto_update,
                                            uint16_t adc_id, 
...
FreeRTOS_VERIFIED_TELEMETRY_DB* sample_nx_verified_telemetry_user_init()
{
    ...
    {
        Serial.printf("Failed to configure Verified Telemetry settings: error code = 0x%08x\r\n", status);
    }
